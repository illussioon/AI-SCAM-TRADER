    ...
    <div v-if="isOpen" class="fixed inset-0 flex items-end justify-center z-[999]" @mousedown="onOverlayMouseDown">
      ...
      <Transition name="drawer-slide">
        ...
        >
          ...
          <!-- Обычный хедер "Кошелек" -->
          <header v-if="!showBankDeposit" class="flex items-center justify-between p-4 -translate-y-2">
          ...
          <!-- Хедер "Банковский платеж" -->
          <BankDepHeader 
            v-if="showBankDeposit" 
            @back="handleShowBankDeposit(false)" 
            @close="close" 
          />
          
          <div style="margin: 0px 13px 0 13px;">
            <TopMenu 
              :show-bank-deposit="showBankDeposit"
              @withdraw-method-select="handleWithdrawMethodSelect" 
              @close="close"
              @show-bank-deposit="handleShowBankDeposit"
            /> 
          </div>
        </div>
...
import BankDepHeader from './bank-dep-header.vue'
...
const emit = defineEmits(['update:modelValue', 'withdraw-method-select']);
...
// Состояние для показа формы банковского депозита
const showBankDeposit = ref(false)

// Обработчик для переключения на форму банковского депозита
const handleShowBankDeposit = (show) => {
  showBankDeposit.value = show
}

// Сбрасываем состояние при закрытии попапа
watch(isOpen, (newValue) => {
  if (!newValue) {
    dragDeltaY.value = 0
    showBankDeposit.value = false // Сброс при закрытии
  }
})
